...
async function downloadImageFromMessage(token: string, message: TelegramMessage): Promise<string | null> {
  ...
  const dir = join(process.cwd(), "claudeclaw", "inbox", "telegram");
...
async function downloadDocumentFromMessage(token: string, message: TelegramMessage): Promise<{ localPath: string; originalName: string } | null> {
