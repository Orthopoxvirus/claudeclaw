...
**CRITICAL: Job files MUST be created in the project-relative path `claudeclaw/jobs/`, NOT in `~/claudeclaw/jobs/`.** The daemon only watches the project directory. Using the home directory path will silently fail — the job will never fire.
...
1. List all `.md` files in `claudeclaw/jobs/`.
...
3. Create the job file at `claudeclaw/jobs/<name>.md` with this exact format:
   ```markdown
   ---
...
1. Read `claudeclaw/jobs/<job-name>.md`. If it doesn't exist, list available jobs and ask the user which one to edit.
...
3. If confirmed, delete `claudeclaw/jobs/<job-name>.md`.
...
1. Read `claudeclaw/jobs/<job-name>.md`. If it doesn't exist, list available jobs.
...
Jobs live in `claudeclaw/jobs/` as markdown files:
