...
1. **List log files**: List files in `claudeclaw/logs/` sorted by modification time (newest first).
...
If `claudeclaw/logs/` doesn't exist or is empty, tell the user there are no logs yet.
